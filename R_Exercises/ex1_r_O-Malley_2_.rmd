...
area <- function(x, y) {
  area = x*y
  return(paste('Thea area of a rectangle of sides', x, 'x', y, 'is', area))
}
...
for (i in 1:10) {
  a = i * (i*2)
  print(paste('The are of an', i, 'x', i*2, 'rectangle is', a))
}
```
...
```{r question 2.1}
credit <- read.table("~/Library/CloudStorage/OneDrive-Personal/001. Documents - Main/023. Programming Tools/025. R Studio/Predictive Analytics/Credit.csv", header = T, sep = ",", row.names = 1)
...
print(income.stats) # confirm correct name application
```
...
```{r question 3.2}
# Income box plot
ggplot(data = credit, aes(y = Income)) +
  geom_boxplot() +
  xlab("Income") +
  ylab("") +
  theme_economist()

# Income based on gender boxplot
ggplot(data = credit, aes(x = Gender, y = Income)) +
  geom_boxplot() +
  xlab("Income") +
  ylab("") +
  theme_economist()
...
ggplot(credit, aes(Rating)) +
  ...
  xlab("Rating") +
  ylab("Frequency") +
  ggtitle("Credit Rating Histogram") +
  theme_economist()

# qqplot
qqnorm(credit$Rating, main = "Credit Rating Q-Q Plot")
...
The data does not appear to be normally distributed because, while there is a large number of data points that intersect or fall very close to the trendline, the majority of data points fall above or well above the trendline, which means the data is not normally distributed. 
...
```{r question 4.1}
lm(Rating ~ Income, data = credit) -> lm.rating
summary(lm.rating)
```
...
```{r question 4.2}
plot(Rating ~ Income, data = credit, xlab = "Income", ylab = "Credit Rating")
abline(lm.rating, col = "red")
```
...
The most influential predictors based on statistical significance are Limit and Cards because they have the lowest p values, with the Limit being the best because it also has the lowest standard error. All variables are statistically significant at p = 0.05 level.
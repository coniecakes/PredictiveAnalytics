...
-   `AirBagDriver only`: $B$ = -4.3447, meaning on average, holding everything else constant, there will be a -4.3447 change in price in relation to `Driver & Passenger`, which is the reference level of the dummy variable for `AirBags`. The negative coefficient value means that when the dummy variable for `AirBagDriver only` = 1, the price decreases, meaning this characterstic is less valuable in terms of price when compared to the reference level. The variable is significant to the model because p (0.025) \< 0.05 confidence level. 

-   `AirBagsNone`: $B$ = -8.9089, meaning on average, holding everything else constant, there will be a -4.3447 change in price in relation to `Driver & Passenger` (reference level). The negative coefficient value means that when the dummy variable for `AirBagsNone` = 1, the price decreases, meaning this characterstic is less valuable in terms of price when compared to the reference level. The variable is statistically significant to the model beacuse p (0.000590) \< 0.05 confidence level.
...
```{r}
credit <- read.table("Homework/Credit.csv.csv", header = T, sep = ",") # read data into data frame
...
Using standardized variables makes it easier to interpret the strenght of realtionships across variables and makes interpreting variables with different scales easier.

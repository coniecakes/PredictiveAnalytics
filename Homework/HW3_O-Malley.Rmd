...
lm.stepwise <- stats::step(lm.fit.large, scope=list(lower=lm.fit.reduced, upper=lm.fit.large),  
                            object = direction="both", test="F") # run stepwise regression
...
ridge.mod <- glmnet::glmnet(x, y, aplha = 0) # fit a ridge regression model
ridge.mod$lamba[1:6] # view the first 6 lambda values
```
